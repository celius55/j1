	...
	function initMap() 
	{
		...
		@foreach ($result as $r)
		  ...
		  var infowindow{{ $r->id }} = new google.maps.InfoWindow({
			content: '<center> <h4>Teren - {{ $r->suprafata_terenului }} (ari)  - {{ $r->sector }}</h4> <img src="/img/{{ $r->foto_1 }}?w=175&h=120&fit=crop">'
		  });
		  ...
		  marker{{ $r->id }}.addListener('click', function() {
			window.location = '{{ url("/") }}/teren/{{ $r->id }}';
		  });
		@endforeach
	}
...
<div class="container">
	<div class="row">
		...
		@else
			<h4 class="page-title">{{ trans('all.terenuri') }}- @if (isset($tip) && $tip == 'arenda') {{ trans('all.arenda') }} @else {{ trans('all.vinzare') }} @endif</h4>
		@endif
				...
				@foreach ($result as $r)
					<div class="col-xs-12 col-md-12 anunt-bloc">
						<div class="col-xs-12 col-md-3">
							<center>
								<a href="{{ url('/') }}/teren/{{ $r->id }}" target="_blank">
									<img src="/img/{{ $r->foto_1 }}?w=175&h=120&fit=crop" class="img-responsive img-thumbnail">
								</a>
							</center>
						</div>
						<div class="col-xs-12 col-md-6">
							<a href="{{ url('/') }}/teren/{{ $r->id }}" class="title-href" target="_blank">
							...
							<ul class="descr-short">
								...
								@if ($r->modul_de_folosinta != 'cimp_gol')
									<li>{{ trans('all.modul_de_folosinta') }}: <span class="descr-short-color">{{ $r->translate(session('applocale'), $r->modul_de_folosinta) }}</span></li>
								@endif
								</div>
								<div class="col-xs-12 col-md-6">
								<li>{{ trans('all.suprafata_terenului') }}: <span class="descr-short-color">{{ $r->suprafata_terenului }} {{ trans('all.ari') }}</span></li>
						...
						<div class="col-xs-12 col-md-3 price">
							...
							<br>
							<a href="{{ url('/') }}/teren/{{ $r->id }}" target="_blank" class="btn btn-default" style="width: 100%; color: #368429;"><i class="fa fa-info-circle"></i> Detalii</a>
							@if ( isset($session_terenuri) )

								<?php $star = 'no'; ?>
								@foreach ($session_terenuri as $s) 
									@if ( $s->id == $r->id )
										<?php $star = 'yes'; ?>
									@endif
								@endforeach
		...
		<div class="col-xs-12 col-md-3" id="cautare-avansata">
			<button id="select-map-show" class="btn btn-success" style="width: 100%"><h4> <i class="fa fa-map"></i> {{ trans('all.harta_terenuri') }} </h4></button>
			...
			@include('page.search.modul-de-folosinta')
			@include('page.search.retele-ingineresti')
