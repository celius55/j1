	...
	{ 
		...
		@foreach ($result as $r)
		  ...
		  var infowindow{{ $r->id }} = new google.maps.InfoWindow({
			content: '<center> <h4>Comercial - {{ $r->suprafata_totala }} (m2)  - {{ $r->sector }}</h4> <img src="/img/{{ $r->foto_1 }}?w=175&h=120&fit=crop">'
		  });
		  ...
		  marker{{ $r->id }}.addListener('click', function() {
			window.location = '{{ url("/") }}/comercial/{{ $r->id }}';
		  });
		@endforeach
	}
...
<div class="container">
	<div class="row">
		...
		@else
			<h4 class="page-title">{{ trans('all.spatii_comerciale') }} - @if (isset($tip) && $tip == 'arenda') {{ trans('all.arenda') }} @else {{ trans('all.vinzare') }} @endif</h4>
		@endif
				...
				@foreach ($result as $r)
					<div class="col-xs-12 col-md-12 anunt-bloc">
						<div class="col-xs-12 col-md-3">
							<center>
								<a href="{{ url('/') }}/comercial/{{ $r->id }}" target="_blank">
									<img src="/img/{{ $r->foto_1 }}?w=175&h=120&fit=crop" class="img-responsive img-thumbnail">
								</a>
							</center>
						</div>
						<div class="col-xs-12 col-md-6">
							<a href="{{ url('/') }}/comercial/{{ $r->id }}" class="title-href" target="_blank">
							...
							<ul class="descr-short">
								...
								@if ($r->bloc_sanitar != 'cimp_gol')
									<li>{{ trans('all.bloc_sanitar') }}: <span class="descr-short-color">{{ $r->bloc_sanitar }}</span></li>
								@endif
								@if ($r->nivelul != 'cimp_gol')
									<li>{{ trans('all.nivelul') }}: <span class="descr-short-color">{{ $r->nivelul }}</span></li>
								@endif
								</div>
								<div class="col-xs-12 col-md-6">
						...
						<div class="col-xs-12 col-md-3 price">
							...
							<br>
							<a href="{{ url('/') }}/comercial/{{ $r->id }}" target="_blank" class="btn btn-default" style="width: 100%; color: #368429;"><i class="fa fa-info-circle"></i> Detalii</a>
							@if ( isset($session_comerciale) )

								<?php $star = 'no'; ?>
								@foreach ($session_comerciale as $s) 
									@if ( $s->id == $r->id )
										<?php $star = 'yes'; ?>
									@endif
								@endforeach
		...
		<div class="col-xs-12 col-md-3" id="cautare-avansata">
			<button id="select-map-show" class="btn btn-success" style="width: 100%"><h4> <i class="fa fa-map"></i> {{ trans('all.harta_comerciale') }}</h4></button>
			...
			{!! Form::open(['id' => 'spatii-comerciale']) !!}
