...
@foreach ($result as $r)
	@section('title')Casă - {{ $r->numar_de_camere }} @if ($r->numar_de_camere == 1) cameră @else camere @endif  - {{ $r->sector }}@endsection
	@section('keywords')
		<?php 
	...
	function initMap() 
	{
		...
		@foreach ($result as $r)
		  ...
		  var infowindow{{ $r->id }} = new google.maps.InfoWindow({
			content: '<center> <h4>Casă - {{ $r->numar_de_camere }} @if ($r->numar_de_camere == 1) cameră @else camere @endif  - {{ $r->sector }}</h4> <img src="/img/{{ $r->foto_1 }}?w=150&h=150&fit=crop">  <br><br> <a href="{{ url("/") }}/casa/{{ $r->id }}" class="btn btn-success">Detalii</button> </center>'
		  });
				...
				<div class="col-md-4">
					...
					<ul class="descr-item">
						...
						@if ($r->numar_de_camere != 'cimp_gol')
							<li>{{ trans('all.numar_de_camere') }}: {{ $r->numar_de_camere }}</li>
						@endif
						@if ($r->numar_de_nivele != 'cimp_gol')
							<li>{{ trans('all.numar_de_nivele') }}: {{ $r->numar_de_nivele }}</li>
						@endif
						...
						@if ($r->balcon != 'cimp_gol')
							<li>{{ trans('all.balcon') }}: {{ $r->balcon }}</li>
						@endif
						@if ($r->bloc_sanitar != 'cimp_gol')
							<li>{{ trans('all.bloc_sanitar') }}: {{ $r->translate(session('applocale'), $r->bloc_sanitar) }}</li>
						@endif
