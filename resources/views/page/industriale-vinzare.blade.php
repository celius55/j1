	...
	function initMap() 
	{
		...
		@foreach ($result as $r)
		  ...
		  marker{{ $r->id }}.addListener('click', function() {
			window.location = '{{ url("/") }}/industrial/{{ $r->id }}';
		  });
		@endforeach
	}
...
<div class="container">
	<div class="row">
		...
		@else
			<h4 class="page-title">{{ trans('all.spatii_industriale') }} - @if (isset($tip) && $tip == 'arenda') {{ trans('all.arenda') }} @else {{ trans('all.vinzare') }} @endif</h4>
		@endif
				...
				@foreach ($result as $r)
					<div class="col-xs-12 col-md-12 anunt-bloc">
						...
						<div class="col-xs-12 col-md-6">
							<a href="{{ url('/') }}/industrial/{{ $r->id }}" class="title-href" target="_blank">
						...
						<div class="col-xs-12 col-md-3 price">
							...
							<br>
							<a href="{{ url('/') }}/industrial/{{ $r->id }}" target="_blank" class="btn btn-default" style="width: 100%; color: #368429;"><i class="fa fa-info-circle"></i> Detalii</a>
							@if ( isset($session_industriale) )

								<?php $star = 'no'; ?>
								@foreach ($session_industriale as $s) 
									@if ( $s->id == $r->id )
										<?php $star = 'yes'; ?>
									@endif
								@endforeach
		...
		<div class="col-xs-12 col-md-3" id="cautare-avansata">
			<button id="select-map-show" class="btn btn-success" style="width: 100%"><h4> <i class="fa fa-map"></i> {{ trans('all.harta_industriale') }}</h4></button>
			...
			{!! Form::open(['id' => 'spatii-industriale']) !!}
